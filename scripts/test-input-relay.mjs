...
local services = {
  UserInputService = { CreateVirtualInput = function() return virtual end, InputBegan = { Connect = conn }, GetFocusedTextBox = function() end },
...
print(out.log .. "|" .. out.held .. "|" .. out.notes .. "|" .. out.performed .. "|" .. out.released)
`;
  ...
  const [log, held, notes, performed, released] = result.stdout.trim().split("|");
  return { log, held, notes, performed, released };
}
