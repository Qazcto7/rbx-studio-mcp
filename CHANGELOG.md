...
- `input`: a button left down by an earlier call whose own cleanup never ran (Studio force-closed, the relay's process killed) used to stay stuck until the next plan happened to press it, paying for the recovery mid-click. Every relay now releases all three mouse buttons before its own plan runs, silently unless one was actually found down, in which case it says so once in `notes` instead of leaving it for a step to explain later.
...
- `playtest`: a pending `play`/`run` start that has been slow for a while and one that is actually stuck read identically -- both just said "keep polling". Past 120s (the same threshold `stop` itself uses to recognize an abandoned launch) the note now says so plainly: this is stuck, not slow, and `stop` will clear it now. Before that point the advice is unchanged ("this is normal under Wine, keep polling").
